...
<%@page import="org.infinite.db.Manager"%>
<%@page import="org.infinite.engines.magic.MagicEngine"%>
<%
	...
	ArrayList<Spell> list = 	(ArrayList<Spell>)Manager.listByQuery("from "+Spell.class.getName() + " i where i.lev > '0' and i.lev >= '"+ (npc.getLevel()-1) + "' and i.lev <= '"+ (npc.getLevel()+1)+ "'");
...
%>
{
    "rows":[
    	<% for(int i=0;i<list.size();i++){
    		...
    		boolean canBuy=( (c.getGold()>=( priceAdj * list.get(i).getPrice()) ) && MagicEngine.canUseSpell(c,list.get(i))	);
    		%>
    		{
    		"id":<%= list.get(i).getId()%>,
        	"data":[
        		"<%= PagesCst.IMG_SPELL_PATH +  list.get(i).getImage() + PagesCst.IMG_SPELL_EXT%>"
        		,
        		"<b><%= list.get(i).getName()%></b><br/>\
        		<i style='font-size:xx-small;'><%=list.get(i).getDesc()%></i><br/>\
        		<%= (list.get(i).getSpelltype()==InfiniteCst.ITEM_TYPE_WEAPON)?"Damage":"Armor Class"%> : <%= list.get(i).getDamage()%><br/>\
